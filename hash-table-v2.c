...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		if (pthread_mutex_init(&entry->te_lock, NULL) != 0) exit(errno);
	}
	return hash_table;
}
                             ...
                             uint32_t value)
{
	...
	list_entry->key = key;
	list_entry->value = value;
	if (pthread_mutex_lock(&hash_table_entry->te_lock) != 0) exit(errno);
	SLIST_INSERT_HEAD(list_head, list_entry, pointers);
	if (pthread_mutex_unlock(&hash_table_entry->te_lock) != 0) exit(errno);
}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		if( pthread_mutex_destroy(&entry->te_lock) != 0) exit(errno);
	}
	free(hash_table);
}
