...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	if (pthread_mutex_init(&lock, NULL) != 0) exit(errno);
	return hash_table;
}
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             ...
                             uint32_t value)
{
	if (pthread_mutex_lock(&lock) != 0) exit(errno);
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	struct list_head *list_head = &hash_table_entry->list_head;
	struct list_entry *list_entry = get_list_entry(hash_table, key, list_head);
	...
	if (list_entry != NULL) {
		...
		if (pthread_mutex_unlock(&lock) != 0) exit(errno);
		return;
	}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	if (pthread_mutex_destroy(&lock) != 0) exit(errno);
	free(hash_table);
}
